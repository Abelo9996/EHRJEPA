...
Usage:
    python generate_sample_data.py --output_dir /path/to/output --num_patients 1000
"""
...
if __name__ == '__main__':
    ...
    print(f"  Train file: {train_file} ({len(train_patients)} patients, {len(train_df)} records)")
    print(f"  Test file: {test_file} ({len(test_patients)} patients, {len(test_df)} records)")


def main():
    parser = argparse.ArgumentParser(description='Generate sample MIMIC-IV-like EHR data')
    parser.add_argument('--output_dir', type=str, 
                       default='./data/sample_ehr',
                       help='Output directory for generated data')
    parser.add_argument('--num_patients', type=int, default=1000,
                       help='Number of patients to generate')
    parser.add_argument('--min_visits', type=int, default=10,
                       help='Minimum visits per patient')
    parser.add_argument('--max_visits', type=int, default=50,
                       help='Maximum visits per patient')
    parser.add_argument('--num_vitals', type=int, default=10,
                       help='Number of vital sign features')
    parser.add_argument('--num_labs', type=int, default=15,
                       help='Number of lab test features')
    ...
    df, output_file = generate_sample_ehr_data(
        ...
        min_visits_per_patient=args.min_visits,
        max_visits_per_patient=args.max_visits,
        num_vital_features=args.num_vitals,
        num_lab_features=args.num_labs,
        output_dir=args.output_dir,
        seed=args.seed
    )
