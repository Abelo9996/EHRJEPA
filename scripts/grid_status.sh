...
cd "$(dirname "$0")/.."
for g in "$@"; do
  rows=$(grep -cE '^\| `' "docs/experiments/$g/summary.md" 2>/dev/null || true); rows=${rows:-0}
  fin=$(grep -c 'ablation finished' "runs/$g/ablate.log" 2>/dev/null || true); fin=${fin:-0}
  err=$(grep -cE 'Traceback|CUDA out of memory|Killed' "runs/$g/ablate.log" 2>/dev/null || true); err=${err:-0}
