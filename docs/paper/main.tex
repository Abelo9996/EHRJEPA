...
the hybrid reads 0.6487 against \code{ar}'s 0.5994. A few-shot run on the
final 1B checkpoints (\code{ar} and \code{hybrid\_final}, Section~\ref{sec:final1b})
is in progress; no result is reported yet.
...
\begin{enumerate}[leftmargin=*,itemsep=3pt,topsep=3pt]
\item \textbf{Every DE-SynPUF number before Section~\ref{sec:final1b}'s table
      used the 3{,}000-subject held-out subset, not the full split.} Table
      \ref{tab:master}, \ref{tab:seeds}, \ref{tab:fewshot},
      \ref{tab:scale}--\ref{tab:ablate4} are all on that subset.
      Section~\ref{sec:final1b}'s full-held-out-split (11{,}708-subject)
      comparison of \code{ar} against the repository default supersedes the
      subset numbers for the cells it covers; it does not exist for every
      cell in the earlier tables (only \code{ar}, the SIGReg-on hybrid and
      \code{hybrid\_final} at 1B tokens were re-scored on the full split).
...
\begin{enumerate}[leftmargin=*,itemsep=2pt,topsep=3pt]
\item A few-shot run on the final 1B checkpoints (\code{ar} and
      \code{hybrid\_final}) is in progress.
