...
hybrid; the 200M budget, and \code{recon\_only} and \code{jepa\_ema} at every
budget they were run at, remain one training seed per cell. Nothing here
...
at every step (0.7287, 0.7328, 0.7363, the last a 3-seed mean) while AR does
not improve from 200M to 1B (0.7304, 0.7251, the last a 3-seed mean). At 1B,
the hybrid leads AR on six of seven tasks by 0.81 to 2.16 AUROC points, and
the per-task min-max range across the three seeds does not overlap between
AR and the hybrid on four of those six. Three pre-registered diagnostics
measured on the
...
reference; the 1B \code{ar}/hybrid points are 3-seed means with a min-max
error bar. Right: per-task AUROC at 1B tokens, \code{ar} against the hybrid,
paired bars, each a 3-seed mean with a min-max whisker.}
\label{fig:scale}
\end{figure}
...
dropped, per the no-SIGReg result above. The target encoder is EMA or a
frozen AR teacher (Section~\ref{sec:ablate3}), pending \code{ablate4-desynpuf}.
...
trainable. \code{ablate4-desynpuf} (running) puts the frozen AR teacher and
$\lambda_{\mathrm{sig}} = 0$ in the same cell, two seeds, to test whether the
two gains stack.
...
hybrid's worst seed (0.6840). And it is the task with the largest few-shot
separation: at $k{=}128$ the hybrid reads 0.6487 against \code{ar}'s 0.5994,
and at 48M with three seeds the hybrid-over-\code{ar} gap (0.6969 vs 0.6824)
exceeds both families' seed ranges. Readmission is also the only task
...
(Section~\ref{sec:ablate2}) --- the hybrid objective, causal encoder, EMA
target, horizons $[1, 4, 16]$, $\lambda_{\mathrm{recon}} = 0.1$,
$\lambda_{\mathrm{sig}} = 0$ (SIGReg dropped) --- is now the default for every
item below that trains a model, unless the item says otherwise.

\begin{enumerate}[leftmargin=*,itemsep=2pt,topsep=3pt]
\item Reseed the 200M grid (\code{ar}, hybrid, \code{recon\_only},
      \code{jepa\_ema}) so the 200M-to-1B step is a matched-seed-count
      comparison, now that 1B has three seeds for \code{ar} and the hybrid.
...
\item Confirm the no-SIGReg default at the 200M/1B scale and, if it holds,
      reseed the 1B hybrid cells at $\lambda_{\mathrm{sig}} = 0$ so
      Table~\ref{tab:scale}'s scaling result is reported under the default
      configuration rather than the $\lambda_{\mathrm{sig}} = 0.05$ setting
      it currently uses.
